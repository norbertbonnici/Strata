...
#
# Versions are pinned (reproducible builds) but overridable via env vars. libyal
# PRUNES its old alpha/experimental releases every month or two, so a pinned tag
# eventually 404s. When that happens, bump the matching LIB*_VERSION to a current
# tag — the newest is the first "tag_name" from
#   curl -fsSL https://api.github.com/repos/libyal/<lib>/releases | grep -m1 tag_name
# (or the GitHub Releases page). Pins last verified building: 2026-07.
...
LIBEVTX_VERSION="${LIBEVTX_VERSION:-20260705}"
LIBREGF_VERSION="${LIBREGF_VERSION:-20260526}"
LIBLNK_VERSION="${LIBLNK_VERSION:-20260525}"
LIBSCCA_VERSION="${LIBSCCA_VERSION:-20260527}"
...
build_arch() {
    ...
    # pkg-config shim. The newer libyal tags (libevtx/libregf/liblnk/libolecf/
    # libscca/…) were generated by an autotools that hard-aborts ("pkg-config not
    # found") when pkg-config is absent - and stock macOS (no Homebrew) has none.
    # A tiny shim claims a recent version (so configure proceeds) and reports every
    # module as absent, making each configure fall back to the deps bundled in its
    # tarball. Exported globally (PKG_CONFIG + PATH) so EVERY lib's configure finds
    # it - not just the one that first needed it.
    local pkgshim="$BUILD/pkgconfig-shim"
    mkdir -p "$pkgshim"
    cat > "$pkgshim/pkg-config" <<'SHIM'
#!/bin/sh
# Minimal offline pkg-config shim (build-tsk.sh): satisfies the version gate;
# reports no modules so configure uses the tarball's bundled dependencies.
for a in "$@"; do
    case "$a" in
...
SHIM
    ...
    export PKG_CONFIG="$pkgshim/pkg-config"
    ...
    # (Its configure hard-requires pkg-config; the shim exported at the top of
    # build_arch satisfies it - see there.)
