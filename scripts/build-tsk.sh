...
TOOLS=(tsk_loaddb fls icat mmls fsstat blkstat istat evtxexport evtxinfo regfexport regfinfo sccainfo ewfinfo ewfverify)
...
build_arch() {
    ...
    # libscca (Windows Prefetch parsing). The alpha tarball vendors the
    # libfwnt dependency that does the Xpress-Huffman decompression Win10/11
    # prefetch needs, so there's nothing extra to build. --without-libfuse
    # drops the sccamount tool we don't ship; we only want sccainfo.
    #
    # Unlike the 2024-era libevtx/libregf, libscca's newer configure was
    # generated by an autotools that hard-aborts ("pkg-config not found") when
    # pkg-config is absent - and stock macOS (no Homebrew) has none. We hand it
    # a tiny shim that claims a recent version (so configure proceeds) and
    # reports every module as absent, which makes configure fall back to the
    # deps bundled in the tarball - exactly what the other libyal libs do when
    # pkg-config is simply missing.
    local pkgshim="$BUILD/pkgconfig-shim"
    if [ ! -x "$pkgshim/pkg-config" ]; then
        mkdir -p "$pkgshim"
        cat > "$pkgshim/pkg-config" <<'SHIM'
#!/bin/sh
# Minimal offline pkg-config shim (see build-tsk.sh). Satisfies the version
# gate; reports no modules so configure uses the bundled dependencies.
for a in "$@"; do
    case "$a" in
...
exit 1
SHIM
        chmod +x "$pkgshim/pkg-config"
    fi
    download "https://github.com/libyal/libscca/releases/download/$LIBSCCA_VERSION/libscca-alpha-$LIBSCCA_VERSION.tar.gz" \
             "$BUILD/libscca.tar.gz"
    extract "$BUILD/libscca.tar.gz" "$BUILD/libscca-$arch"
