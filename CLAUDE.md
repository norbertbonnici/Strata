...
| `StrataTimeline` | MACB timeline + per-artifact timeline projections (28 sources) + gap/session analysis |
...
| `StrataAnalysis` | `Analyzer` protocol, `AnalysisEngine`, **55 analyzers**, IOC matcher, lateral graph, `CorrelationEngine` (case-wide multi-host: shared IOC / pivoting source IP / reused account across ≥2 hosts) |
...
- **Per-OS tab hiding.** Artifact tabs that can't apply to the evidence's OS are
  ...
  Carved Files + Extensions + Background Items + Messages + Mail, `nil` =
...
registry + prefetch + Amcache/Shimcache + LNK + JumpList + USN-journal + SRUM + browser-history + `$MFT` + WMI-persistence parsing → host profile, 55 ATT&CK analyzers → kill chain, IOC matching,
