...
| `StrataAnalysis` | `Analyzer` protocol, `AnalysisEngine`, **53 analyzers**, IOC matcher, lateral graph, `CorrelationEngine` (case-wide multi-host: shared IOC / pivoting source IP / reused account across ≥2 hosts) |
...
- **Per-OS tab hiding.** Artifact tabs that can't apply to the evidence's OS are
  ...
  Carved Files + Extensions + Background Items, `nil` = cross-platform incl.
...
registry + prefetch + Amcache/Shimcache + LNK + JumpList + USN-journal + SRUM + browser-history + `$MFT` + WMI-persistence parsing → host profile, 53 ATT&CK analyzers → kill chain, IOC matching,
