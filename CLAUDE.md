...
| `StrataAnalysis` | `Analyzer` protocol, `AnalysisEngine`, **60 analyzers**, IOC matcher, lateral graph, `CorrelationEngine` (case-wide multi-host: shared IOC / pivoting source IP / reused account across ≥2 hosts) |
...
- **Per-OS tab hiding.** Artifact tabs that can't apply to the evidence's OS are
  ...
  Powerlog + Configuration + Installs, `nil` = cross-platform incl.
  **Browser History**, always shown).
...
registry + prefetch + Amcache/Shimcache + LNK + JumpList + USN-journal + SRUM + browser-history + `$MFT` + WMI-persistence parsing → host profile, 60 ATT&CK analyzers → kill chain, IOC matching,
